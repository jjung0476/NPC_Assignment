...
public class SeaUrchinController : MonoBehaviour
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.name == "Player")
        {
            ...
            // [자동 찾기] 화면에 꺼져있는 모든 오브젝트 중에서 "gameover"라는 이름을 가진 녀석을 찾아옵니다.
            GameObject[] allObjects = Resources.FindObjectsOfTypeAll<GameObject>();
            foreach (GameObject obj in allObjects)
            {
                // 이름이 "gameover"이고, 실제로 게임 화면(씬)에 존재하는 녀석이라면?
                if (obj.name == "gameover" && obj.scene.name != null)
                {
                    obj.SetActive(true); // 화면에 켜기!
                    break; // 찾았으니 반복문 종료
                }
            }

            // 이미지를 켠 "직후"에 유니티 시간의 흐름을 0으로 만들어 게임을 일시정지 시킵니다.
            Time.timeScale = 0f;
        }
    }
}